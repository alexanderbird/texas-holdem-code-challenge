...
describe('highCard', () => {
  it('matches the card with the highest value when they are all different', () => {
    const match = highCard([ club.two, heart.four, diamond.six, spade.three, club.five ]) as HandClassificationCardPart;
    expect(match.scoringCards).toEqual([ diamond.six ]);
  });

  it('matches the ace given 10, J, Q, K, A', () => {
    const match = highCard([ diamond.ten, spade.king, club.jack, heart.ace, diamond.queen ]) as HandClassificationCardPart;
    expect(match.scoringCards).toEqual([ heart.ace ]);
  });
